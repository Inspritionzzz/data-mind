...
rcl = [1, 2, 3, 4, 5]
sy = ['1990-2005', '2005-2010', '2010-2011', '2011-2012', '2012-2013', '2013-2014', '2014-2015', '2015-2016',
...
for pr in provs:
    for lv in rcl:
        for ar in list(wdl):
            url0 = 'https://aiqicha.baidu.com/s?q=%s&f={"provinceCode":"%d","regCapLevel":"level%d","industryCode1":"%s"}' % (
            kw, pr, lv, ar)
            resp = req_get(url0)
            ...
            # 大于1000条，进行拆分检索
                ...
                for yr in sy:
                    url = 'https://aiqicha.baidu.com/s?q=%s&f={"provinceCode":"%d","regCapLevel":"level%d","industryCode1":"%s","startYear":"%s"}' % (
                    kw, pr, lv, ar, yr)
                    try:
                        resp = req_get(url)
                        write_comp(resp)
