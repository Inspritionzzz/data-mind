...
class Notifier:
    ...
    def is_participated(self, dynamic_id):
        ...
        cur = self.conn.execute(
            'select 1 from lottery_record where dynamic_id = ?', (str(dynamic_id),))
    ...
    def notify(self, summary_records):
        ...
        for lottery, result in summary_records:
            ...
            lines.append(
                ...
                f"动态 {lottery['dynamic_id']} 条件:{','.join(sorted(lottery.get('conditions', [])))} "
                f"开奖:{lottery.get('deadline') or '未知'}")
        content = '\n'.join(lines)
        logger.info('汇总通知:\n%s', content)
        self._send_mail('B站抽奖机器人运行报告', content)
