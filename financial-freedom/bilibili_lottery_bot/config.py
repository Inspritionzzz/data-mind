...
# 代理抽奖账号：其动态本身不是抽奖（直接转发无效），
# 动态里的链接（专栏）才是抽奖信息集合，需解析后参与其中列出的抽奖
PROXY_ACCOUNTS = {'你的抽奖工具人'}   # 按用户名匹配
PROXY_ACCOUNT_UIDS = {'100680137'}   # 按 uid 匹配
PROXY_MAX_TARGETS = 10               # 每篇代理专栏最多处理的抽奖 UP 数

# ---------- 执行层 ----------
REPOST_TEXTS = ['拉低中奖率', '分母+1', '求中奖！', '转发动态', '冲冲冲，让我中一次']
# 注意：'拉低中奖率'/'分母来了'/'来了来了' 等抽奖黑话已被B站评论风控过滤
# （接口返回 code=0 假成功但评论不可见），只能使用经验证可见的文案
COMMENT_TEXTS = [
    '求中奖！', '支持一下，坐等开奖', '蹲一个好运', '参与一下，沾沾喜气',
    '冲鸭，希望有我', '许愿中奖', '在线等一个欧气',
]
