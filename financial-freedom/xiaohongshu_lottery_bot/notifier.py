...
class Notifier:
    ...
    # ---------- 数据库记录 ----------
    def is_participated(self, note_id):
        """判断该笔记是否已参与过（去重）"""
        cur = self.conn.execute(
            'select 1 from lottery_record where note_id = ?', (str(note_id),))
    ...
    def record(self, lottery, exec_result):
        ...
        try:
            self.conn.execute(
                ...
                '(note_id, user_id, nickname, conditions, deadline, '
                ' actions, success, created_at) values (?, ?, ?, ?, ?, ?, ?, ?)',
                (str(lottery['note_id']), str(lottery['user_id']), lottery['nickname'],
    ...
    def notify(self, summary_records):
        ...
        for lottery, result in summary_records:
            ...
            lines.append(
                f"- [{status}] {lottery['nickname']}(uid={lottery['user_id']}) "
                f"笔记 {lottery['note_id']} 条件:{','.join(sorted(lottery.get('conditions', [])))} "
                f"开奖:{lottery.get('deadline') or '未知'}")
        content = '\n'.join(lines)
        logger.info('汇总通知:\n%s', content)
        self._send_mail('小红书抽奖机器人运行报告', content)
